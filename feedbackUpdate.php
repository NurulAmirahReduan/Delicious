...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($email_err) && empty($comment_err)){
        ...
        $sql = "UPDATE user_feedback SET name=?, email=?, comment=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_email, $param_comment, $param_id);
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    header("location: feedbackError.php");
                    exit();
                }
